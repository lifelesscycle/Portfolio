...
/* Same engine as the reference LaserFlow demo, with one structural change:
   the original always attached pointer listeners and only neutralized
   their effect via `mouseTiltStrength`. Here, `followMouse` (default
   false) gates whether those listeners are attached at all, matching how
   Galaxy/LightRays/LightPillar already behave — so "does this track the
   cursor" is a single, consistent switch across all four background
   effects instead of a per-shader convention. */
const LaserFlow = ({
  className,
  style,
  wispDensity = 1,
